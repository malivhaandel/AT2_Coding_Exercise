 ...
 *
 * click memory nodes and geometry
 * extra points on new nodes
 */
...
let placedStars = []; // stores user-clicked star nodes permanently
...
// returns current active color palette based on click
...
function setup() {
  ...
  buildSparkles();      // initialise background sparkle system
  buildPhrasePoints();  // generate first text-to-point layout
}

// creates floating sparkle particles across the canvas
function buildSparkles() {
  for (let i = 0; i < 200; i++) {
    sparkles.push({
      x: random(width),
      y: random(height),
      r: random(4, 12),              // base size of sparkle
      alpha: random(0, 255),         // transparency
      twinkle: random(0.02, 0.08),   // flicker speed
      phase: random(TWO_PI)          // animation offset
    });
  }
}

// converts current phrase into vector point cloud + centers it
function buildPhrasePoints() {
  ...
  // second-pass centering for accuracy
  ...
  let offsetY = height / 2 - (minY + maxY) / 2 + 60;
...
// click: cycle palette, change phrase, add permanent star node
...
function draw() {
  ...
  // connection system (mouse-centered)
